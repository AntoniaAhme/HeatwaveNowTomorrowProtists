...
# Wobble out time through GAM and extract residuals
require(gam)
div_det <- gam(Shannon ~ s(time,7), data=div)
plot(div_det)
div_det$aic
div$res <- div_det$residuals
head(div)
...
shan_time <- ggplot(div, aes(x=time, y=res, color=Treatment)) + 
  geom_point(position=position_dodge(0.05), size = 3)+
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs", fx = TRUE, k = 5), se=F, size = 1)+
  plot.theme +
  labs(x="Incubation time (d)", y=bquote("Residuals")) + 
...
# Wobble out time through GAM and extract residuals
require(gam)
div_det <- gam(Shannon ~ s(time,4), data=div)
div$res <- div_det$residuals
