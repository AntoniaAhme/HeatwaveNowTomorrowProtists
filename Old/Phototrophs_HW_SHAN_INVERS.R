...
treat_pal <- c("AMB+HW"="darkorange", "FUT"="blue", "FUT+HW"="turquoise3") # inverted for presentation
treat_pch <- c(16,17)
...
hw_time <- ggplot(hw, aes(x=time, y=LRR, color = Treatment, shape = Treatment)) +
  ...
  geom_line(data = signi_fut_hw, aes(time, LLRmin), size = 3, color = "turquoise3") +
  ...
  geom_line(data = signi_amb1, aes(time, LLRmin), size = 3, color = "darkorange1") +
  geom_line(data = signi_amb2, aes(time, LLRmin), size = 3, color = "darkorange1") +
...
hw_time
ggsave("Output/PP_HW_LRR_invers.png", hw_time, height = 4, width = 8, dpi = 320, bg="transparent")
